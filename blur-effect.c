...
void* function(void *arg){
  ...
  if(tinfo->size == 3 || tinfo->size == 7 || tinfo->size == 5 || tinfo->size == 9 || tinfo->size == 11 || tinfo->size == 13 || tinfo->size == 15){
	int m,w2;
     for( m = tinfo->begin2; m < tinfo->end2; m++){
...
int main(int argc, char *argv[]) {
  ...
  tinfo = calloc(hilos,sizeof(struct thread_info));
  if(tinfo == NULL)
    perror("Calloc");
 ...
 for(tnum = 0; tnum < hilos  ; tnum++){

				 if ( tnum != 0 ) {
					 if (end2 + divi + fin  > height){
						 begin2 = end2;
  					 end2 = height- fin;
						}
          	else{
          ...
          tinfo[tnum].size= nrows;
          tinfo[tnum].begin2 = begin2;
          tinfo[tnum].end2 = end2;

          id = pthread_create(&tinfo[tnum].thread_id,NULL,&function,&tinfo[tnum]);


					//printf("hilo %i begin  %i   end %i  \n",tnum,begin2,end2);
    ...
    for(tnum = 0; tnum < hilos ; tnum++){
          id = pthread_join(tinfo[tnum].thread_id,NULL);
